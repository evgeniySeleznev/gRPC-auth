...
import (
	...
	"github.com/evgeniySeleznev/gRPC-auth/tests/suite"
...
func TestRegisterLogin_Login_HappyPath(t *testing.T) {
	...
	respReg, err := st.AuthClient.Create(ctx, &auth_v1.CreateRequest{
		Email:    email,
		Password: pass,
	})
	require.NoError(t, err) //нет смысла дальше что-то проверять, если здесь ловим ошибку
	assert.NotEmpty(t, respReg.GetId())

	respLogin, err := st.AuthClient.Get(ctx, &auth_v1.GetRequest{
		Email:    email,
		Password: pass,
		AppId:    appID,
	})
...
func TestRegisterLogin_DuplicatedRegistration(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotEmpty(t, respReg.GetId())
...
func TestRegister_FailCases(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := st.AuthClient.Create(ctx, &auth_v1.CreateRequest{
				Email:    tt.email,
...
func TestLogin_FailCases(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err = st.AuthClient.Get(ctx, &auth_v1.GetRequest{
				Email:    tt.email,
				Password: tt.password,
				AppId:    tt.appID,
			})
